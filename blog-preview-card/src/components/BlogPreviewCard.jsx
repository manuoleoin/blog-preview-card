...
export default function BlogPreviewCard({ post }) {
    const { coverImage, categories, publishedAt, title, excerpt, author } =
        post;
    return (
        <article className="border-gray-950 bg-white flex w-full max-w-96 flex-col gap-6 rounded-[20px] border p-6 shadow-[8px_8px_0_var(--color-gray950)] transition-shadow hover:shadow-[16px_16px_0_var(--color-gray-950)]">
...
font-extrabold"
                ...
                <h1 className="text-gray-950 hover:text-yellow cursor-pointer text2xl font-extrabold transition-colors">
                    {title}
                    11.4 APágina
                    38
                </h1>
